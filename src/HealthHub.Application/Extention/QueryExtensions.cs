...
namespace HealthHub.Application.Extention
{
    public static class QueryExtensions
    {
        ...
        {
            public async Task<T> FirstOrErrorAsync(Guid id, Exception? exception = null)
            ...
            public IQueryable<T> Paginate<TDto>(GlobalFilter<TDto> filter) where TDto : new()
                => source
                    .Skip(filter.Pagination.PageNo * filter.Pagination.RowsCount)
                    .Take(filter.Pagination.RowsCount);

            public IQueryable<T> OrderAndPaginate<TDto>(GlobalFilter<TDto> filter) where TDto : new()
                 => source.OrderWith(filter).Paginate(filter);


            public IQueryable<T> OrderWith<TDto>(GlobalFilter<TDto> filter) where TDto : new()
            {
                if (string.IsNullOrEmpty(filter.SortField))
                    filter.SortField = nameof(BaseEntity.CreatedOn);

                return
                    filter.SortApproach == SortingTypeEnum.Desc ?
